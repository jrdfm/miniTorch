...
class BatchNorm1d(Module):
    ...
    def forward(self, x):
        ...
        if self.is_train == True:
            ...
            sig = ((x - mean) ** 2).sum(axis = 0) / Tensor(x.shape[0] - 1)
...
class BatchNorm2d(Module):
    ...
    def __init__(self, num_features, eps=1e-5, momentum=0.1):
        ...
        # To make the final output affine
        self.gamma = Tensor(np.ones((self.num_features,)), requires_grad=True, is_parameter=True).reshape(shape=[1, -1, 1, 1])
        self.beta = Tensor(np.zeros((self.num_features,)), requires_grad=True, is_parameter=True).reshape(shape=[1, -1, 1, 1])
    ...
    def forward(self, x):
        ...
        """
        if self.is_train == True:
            mean = x.mean(axis = (0, 2, 3)).reshape(shape=[1, -1, 1, 1])
            var = ((x - mean) ** 2).mean(axis = (0, 2, 3)).reshape(shape=[1, -1, 1, 1])
            sig = (((x - mean) ** 2).sum(axis = (0,2,3))/ F.tensorize((np.prod([x.shape[i] for i in [0, 2, 3]])-1)))
            x_hat = (x - mean) / (var + self.eps)** Tensor(0.5)
            self.running_mean = (Tensor(1) - self.momentum)* self.running_mean + self.momentum* mean
            self.running_var =  (Tensor(1) - self.momentum)* self.running_var + self.momentum* sig
        else:
