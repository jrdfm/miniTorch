...
def test_maxpool2d_forward():
    ...
    for num_layers in range(1, 2):
        ...
        in_c = np.random.randint(2, 5)
        channels = [np.random.randint(2, 5) for i in range(num_layers + 1)]
        kernel = [np.random.randint(3, 7) for i in range(num_layers)]
        stride = [np.random.randint(1, 3) for i in range(num_layers)]
        
        # Init model with above params
        mytorch_layers = []
        for i in range(num_layers):
            mytorch_layers.append(Conv2d(channels[i], channels[i+1], kernel[i], stride[i]))
            mytorch_layers.append(MaxPool2d(kernel_size=kernel[i], stride=stride[i]))
        mytorch_model = Sequential(*mytorch_layers)
        
        # Run just the model forward
        if not test_forward(mytorch_model):
            return False
    return True
...
def test_maxpool2d_backward():
    ...
    for num_layers in range(1, 2):
        ...
        for i in range(num_layers):
            ...
            mytorch_layers.append(MaxPool2d(kernel_size=kernel[i], stride=stride[i]))
        mytorch_model = Sequential(*mytorch_layers)
        mytorch_optimizer = SGD(mytorch_model.parameters())

        if not test_step(mytorch_model, mytorch_optimizer, 5):
...
def test_avgpool2d_forward():
    ...
    for num_layers in range(1, 2):
        ...
        for i in range(num_layers):
            mytorch_layers.append(Conv2d(channels[i], channels[i+1], kernel[i], stride[i]))
            mytorch_layers.append(AvgPool2d(kernel_size=kernel[i], stride=stride[i]))
        mytorch_model = Sequential(*mytorch_layers)
...
# def test_forward(mytorch_model, in_features, mytorch_criterion=None, batch_size=None, width=None):
#     """
#     Tests forward, printing whether a mismatch occurs in forward or backwards.
...
#     """
#     pytorch_model = get_same_pytorch_mlp(mytorch_model)
#     batch_size = batch_size if not batch_size is None else np.random.randint(1, 3)
#     width = width if not width is None else np.random.randint(15, 25)
#     x, y = generate_dataset_for_mytorch_model(mytorch_model, width, batch_size, in_features)
#     pytorch_criterion = get_same_pytorch_criterion(mytorch_criterion)
#     forward_passed, _ = forward_(mytorch_model, mytorch_criterion, pytorch_model, pytorch_criterion, x, y)
#     if not forward_passed:
#         print("Forward failed")
#         return False
...
# def test_step(mytorch_model, mytorch_optimizer, steps, in_features,
#               mytorch_criterion=None, batch_size=None,
#               pytorch_model=None, width=None):
#     """
#     Tests subsequent forward, back, and update operations, printing whether
...
def get_same_pytorch_mlp(mytorch_model):
    ...
    for l in mytorch_model.layers:
        if isinstance(l, Linear):
            ...
            layers.append(nn.MaxPool2d(kernel_size=l.kernel_size, stride=l.stride))
        elif isinstance(l, AvgPool2d):
            layers.append(nn.AvgPool2d(kernel_size=l.kernel_size, stride=l.stride))
        else:
...
#     Sequential model.
#     """
#     return get_mytorch_pool_layers(mytorch_model)[0].in_channel
...
#     Sequential model.
#     """
#     return get_mytorch_pool_layers(mytorch_model)[-1].out_channel

# def get_mytorch_pool_layers(mytorch_model):
#     """
#     Returns a list of linear layers for a mytorch model.
#     """
#     return list(filter(lambda x: isinstance(x, MaxPool2d), mytorch_model.layers)) + list(filter(lambda x: isinstance(x, AvgPool2d), mytorch_model.layers))

# def get_pytorch_pool_layers(pytorch_model):
#     """
#     Returns a list of linear layers for a pytorch model.
#     """
#     return list(filter(lambda x: isinstance(x, nn.MaxPool2d), pytorch_model)) + list(filter(lambda x: isinstance(x, nn.AvgPool2d), pytorch_model))
