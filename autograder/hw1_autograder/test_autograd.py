...
def test_pow():
    """Tests that mytorch power matches torch's power"""
    np.seterr(invalid='ignore') # suppress RuntimeWarning: invalid value encountered in power
    # shape of tensor to test
    shape = (1, 2, 3)
    ...
    # run mytorch and torch forward: 'c = a + b'
    ctx = ContextManager()
    c = Pow.forward(ctx, a, b)
    c_torch = a_torch ** b_torch

    # run mytorch and torch addition backward
    back = Pow.backward(ctx, Tensor.ones(*shape))
    c_torch.sum().backward()
...
def test_exp():
    ...
    # run mytorch and torch forward: 'c = exp(a)'
    ctx = ContextManager()
    c = Exp.forward(ctx, a)
    c_torch = torch.exp(a_torch)

    # run mytorch and torch addition backward
    back = Exp.backward(ctx, Tensor.ones(*shape))
    c_torch.sum().backward()
...
def check_val(mytorch_tensor, pytorch_tensor, eps = 1e-10):
    ...
    if max_diff < eps or np.isnan(max_diff):
        return True
    else:
